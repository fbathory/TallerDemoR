...
eder_ned_sexo <- eder_personas %>% 
  filter(p2 %in% 1:2, # Elimino los que no tienen asignado el sexo y el nivel educativo
         e_nivel != 9) |> 
  mutate(sexo = case_when(p2 == 1 ~ "Varón", # Recodifico
                          p2 == 2 ~ "Mujer"),
         e_nivel_desc = case_when(e_nivel == 0 ~ "No corresponde/asiste o asistió a escuelas especiales no primarias",
                                  e_nivel == 1 ~ "Inicial",
                                  ...
                                  e_nivel == 7 ~ "Superior / Universitario completo y más",
                                  e_nivel == 8 ~ "Sin instrucción")) |> 
  ...
  mutate(e_nivel_desc = factor(e_nivel_desc,
                               levels = c("No corresponde/asiste o asistió a escuelas especiales no primarias",
                                          "Inicial",
                                          "Primario incompleto",
                                          "Primario completo",
                                          ...
                                          "Superior / Universitario completo y más",
                                          "Sin instrucción"))) |> 
...
eder_ned_sexo_long <- eder_ned_sexo |> 
  pivot_longer(cols = c("Mujer", "Varón"), names_to = "Sexo",values_to ="n")
...
g_ned_sexo <- eder_ned_sexo_long %>% 
  ...
  labs(y = "n", 
  x = "Nivel educativo",
  title = "Población por sexo y nivel educativo",
  subtitle = "EDER CABA",
  caption = "Fuente: en base a IDECBA") +
  theme_bw() +
  theme_dark() +
