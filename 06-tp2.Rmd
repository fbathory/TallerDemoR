...
* Crear un *Script* que procese la información necesaria y agregar breves lecturas en forma de comentarios.
...
Primero debes leer los datos censales directo desde R. En todos los casos es el archivo xlsx referido al Cuadro 4, que contiene población por sexo y edad. Va una ayuda para el caso de Total País, donde previamente ingresamos a la sección de [total país](https://censo.gob.ar/index.php/datos_definitivos_total_pais/), descargamos el archivo (en el caso de las provincias, cambia el nombre del archivo xlsx) y lo colocamos en la carpeta data:
```{r, message=F, warning=F}
library(readxl)
total_pais_c2022 <- read_xlsx("data/c2022_tp_est_c4.xlsx", sheet = 3, range = "A4:E127")
total_pais_c2022 <- total_pais_c2022 %>% 
...
Luego agregar:
    - Un párrafo inicial donde se explicite el objetivo del reporte como comentario. Incluir dato sobre la cantidad de población total y su peso en el total país.
