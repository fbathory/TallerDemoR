...
eder_personas_br <- read.csv("data/EDER_CABA/eder2019_personas.csv")
```
...
Previo a comenzar debes haber leído la base de la EDER *"eder2019_personas.csv"*, haberla inspeccionado brevemente y guardado en un objeto llamado _eder_personas_br_ (ver \@ref{intro}). 
...
```
¿9999? Eso no está bien. Volveremos sobre esto más tarde. 
...
Mediante `mutate` podemos crear variables nuevas que pueden o no relacionarse a las existentes. Por ejemplo, hacemos una variable que nos diga si la persona es mayor de edad `es_mayor`:
...
eder_mayores <- eder_personas %>% mutate(es_mayor = p3b >= 18)
...
Si solo querés mostrar las tres personas más añosas, podés utilizar `slice` para seleccionar filas (ya que están ordenadas):

```{r}
eder_mayores %>% slice(1:3) %>% select(id, p3b, edad_altern)
```
...
Pero esto no es tan útil. Entonces podemos hacer la distribución por cohorte de nacimiento agrupada, *recodificando* el año de nacimiento con la función `case_when` en una nueva variable.
...
a) En una sola sentencia obtén la edad mediana nivel educativo de las personas que contestaron la encuesta (utilizando `filter(retro == 1)`)
